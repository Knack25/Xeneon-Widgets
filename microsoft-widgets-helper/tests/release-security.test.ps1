...
Assert-True (($dependencyAuditorSource | Select-String -AllMatches 'npm\s+audit\s+--audit-level=low').Matches.Count -eq 2) 'Dependency auditing must fail on low-or-higher npm advisories in both npm roots.'
Assert-True ($dependencyAuditorSource -match 'low-or-higher') 'The npm audit policy must be documented as low-or-higher, matching audit-level=low.'
