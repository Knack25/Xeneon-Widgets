...
`Planner/PlannerIntegration.cs` registers Planner services and routes. New clients can use `/api/planner/plans`, `/api/planner/display`, `/api/planner/tasks`, and the other Planner routes under this prefix. Existing unprefixed endpoints remain supported for installed widgets. `/display/cached` and its prefixed alias return the selected board's saved snapshot without contacting Microsoft, while the normal display route performs the live refresh. `/board/index.html` remains available and bundles the filtering and view-state modules from the sibling Planner widget project before starting the app.

The Planner widget can edit title, progress, priority, start and due dates, assignees, bucket, existing labels, notes, and checklist content; task creation supports the same applicable organization fields. Structured filters and My tasks are stored per plan in helper settings. Horizontal board and per-bucket vertical offsets remain lightweight widget view state, so the helper stays the sole persistent owner of Planner task data. The widget renders the cached board first, restores its saved view, and applies live data without closing dialogs or replacing drafts. A failed live refresh leaves the cached board visible as an offline view. Outlook fast resume is not part of this change and remains future work.

The Planner connection uses delegated Microsoft Graph permissions `User.Read`, `Tasks.ReadWrite`, and `Group-Conversation.ReadWrite.All`. The organization, filtering, persistence, and cached-resume features add no permission or organizational-consent requirement. Existing users may need to open setup and select **Enable task chat** once to grant the conversation permission. Without it, only task chat is unavailable; Planner boards, tasks, checklists, and notes continue to work, and notes use `Tasks.ReadWrite`. Optional `User.ReadBasic.All` and `GroupMember.ReadBasic.All` permissions enable assignee names and board-member selection respectively.
