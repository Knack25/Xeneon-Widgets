...
export class RefreshState {
  generation=0; resolvedGeneration=0; key=''; cache=new Map(); events=[]; sources=[]; offline=false; authorized=true; controllers=new Set();
  ...
  seed(t,result,now=Date.now()) {
    this.rejectAuthorizationError(result);
    if(!this.current(t) || this.resolvedGeneration===t.generation)return false;
    return this.apply(t,result,now,false);
  }
  accept(t,result,now=Date.now()) {this.rejectAuthorizationError(result);return this.apply(t,result,now,true);}
  rejectAuthorizationError(result) {
    const auth=(result.sources||[]).find(s=>['sign_in_required','consent_required','account_changed','unauthorized'].includes(s.error?.code));
    if(!auth)return;
    this.clearAuthorization();const error=new Error(auth.error.message||'Reconnect Outlook in helper setup.');error.status=401;error.code=auth.error.code;throw error;
  }
  ...
  clearAuthorization(){
    this.authorized=false;this.generation++;this.key='';
