...
public sealed class OutlookAccessService(WidgetPairingService pairing, OutlookAccountState state)
{
    ...
    public Task<PairingResult> PollAsync(string id, string secret, OutlookAccountLease lease, CancellationToken ct) => pairing.PollAsync(id, secret, lease, ct);
    public Task ApproveAsync(string id, CancellationToken ct) => pairing.ApproveAsync(id, ct);
    public Task ApproveAsync(string id, OutlookAccountLease lease, CancellationToken ct) => pairing.ApproveAsync(id, lease, ct);
    public Task RevokeAsync(string id, CancellationToken ct) => pairing.RevokeAsync(id, ct);
    public Task RevokeAsync(string id, OutlookAccountLease lease, CancellationToken ct) => pairing.RevokeAsync(id, lease, ct);
